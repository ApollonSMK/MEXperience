...
export function AdminAppointmentForm({ users, services, plans, onSubmit, onCancel, allTimeSlots, initialTime, initialData }: AdminAppointmentFormProps) {
  ...
  useEffect(() => {
      ...
      if (formType === 'blocked') {
          ...
          if (form.getValues('userId') === 'blocked-admin-placeholder') form.setValue('userId', '');
          if (form.getValues('serviceId') === 'blocked-service-placeholder') form.setValue('serviceId', '');
      }
  }, [formType, form, initialData]);
      ...
      <form onSubmit={form.handleSubmit(onSubmit)} className="flex flex-col gap-4 h-full">
        ...
        {!initialData && (
            <Tabs value={formType} onValueChange={(v) => setFormType(v as any)} className="w-full">
                <TabsList className="grid w-full grid-cols-2">
                    ...
                    <TabsTrigger value="blocked">Bloquer un créneau</TabsTrigger>
                </TabsList>
            </Tabs>
        )}
        ...
        {formType === 'appointment' ? (
                                    ...
                                    render={({ field }) => (
                                        <FormItem className="flex flex-col">
                                            <div className="space-y-2">
                                                {!field.value || field.value === 'blocked-admin-placeholder' ? (
                                                    <Button
                                                        type="button"
                                                        variant="outline"
                                                        className={cn("w-full justify-start text-left h-10 text-sm", (!field.value || field.value === 'blocked-admin-placeholder') && "text-muted-foreground")}
                ...
                <div className="space-y-3">
                        ...
                        render={({ field }) => (
                            <FormItem>
                                ...
                                }} value={field.value === 'blocked-service-placeholder' ? '' : field.value}>
                                    <FormControl>
            ...
            <div className="space-y-6 pt-4">
                <div className="bg-muted/40 p-4 rounded-lg border border-dashed space-y-4">
                     <FormField
                        control={form.control}
                        name="blockReason"
                        render={({ field }) => (
                            <FormItem>
                                <div className="flex items-center gap-2 mb-2">
                                    ...
                                    <FormLabel>Raison de l'indisponibilité</FormLabel>
                                </div>
                                <FormControl>
                                    <Input placeholder="Ex: Pause déjeuner, Réunion..." {...field} />
                                </FormControl>
                                <FormMessage />
                            </FormItem>
                        )}
            ...
            <div className="flex gap-2">
                    ...
                    render={({ field }) => (
                        <FormItem className="flex-1">
                            <Select onValueChange={field.onChange} value={field.value}>
                                <FormControl>
                                    <SelectTrigger className="h-9 text-sm">
                                        <SelectValue placeholder={formType === 'blocked' ? "De" : "Heure"} />
                                    </SelectTrigger>
                                </FormControl>
                                <SelectContent className="h-[200px]">
                                    {allTimeSlots.map((time) => (
                                        <SelectItem key={time} value={time} className="text-sm">
                                            {formType === 'blocked' ? `De ${time}` : time}
                                        </SelectItem>
                                    ))}
                                </SelectContent>
                            </Select>
                ...
                />
                 <FormField
                    ...
                    name="duration"
                    render={({ field }) => (
                        <FormItem className={cn("w-32", formType === 'blocked' && "flex-1")}>
                            {formType === 'appointment' ? (
                                <Select
                                ...
                                /* BLOCKED MODE: END TIME SELECTOR */
                                <Select 
        ...
        <div className="flex flex-col-reverse sm:flex-row justify-end gap-2 pt-6 mt-auto border-t bg-background sticky bottom-0 z-10 -mx-6 px-6 pb-4">
            ...
            <Button type="submit" disabled={form.formState.isSubmitting} className={cn("h-9 text-xs", formType === 'blocked' && "bg-slate-700 hover:bg-slate-600")}>
                {form.formState.isSubmitting ? (
                    ...
                    <>
                        {formType === 'blocked' ? <Ban className="mr-2 h-3.5 w-3.5" /> : <Check className="mr-2 h-3.5 w-3.5" />}
                        {initialData 
                            ? 'Modifier' 
                            : (formType === 'blocked' ? 'Bloquer le créneau' : 'Confirmer le RDV')
                        }
                    </>
                )}
            </Button>
