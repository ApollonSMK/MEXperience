                    ...
                    <div className="w-[350px] shrink-0 border-l bg-slate-50 flex flex-col shadow-inner z-10">
                         ...
                         <div className="flex-1 overflow-y-auto px-6 py-2">
                             ...
                             <div className="space-y-2 text-sm">
                                ...
                                {/* Gift Card Input Popover */}
                                <div className="flex justify-between items-center group">
                                     <span className="text-muted-foreground group-hover:text-slate-900 transition-colors">Carte Cadeau</span>
                                     <div className="flex items-center gap-1">
                                         {appliedGiftCard ? (
                                             <div className="flex items-center text-pink-600 mr-1 bg-pink-50 px-1.5 py-0.5 rounded text-xs font-medium">
                                                 -{appliedGiftCard.amountToUse.toFixed(2)} €
                                                 <button onClick={removeGiftCard} className="ml-1 hover:text-pink-800"><X className="h-3 w-3"/></button>
                                             </div>
                                         ) : null}
                                         ...
                                         <Popover>
                                            ...
                                            <PopoverContent className="w-72 p-0" align="end">
                                                <div className="p-3 space-y-3 border-b">
                                                    <h4 className="font-medium text-sm">Appliquer une carte cadeau</h4>
                                                    <div className="flex gap-2">
                                                        <Input 
                                                            placeholder="Code (ex: GIFT-123)"
                                                            value={giftCardCode}
                                                            onChange={(e) => setGiftCardCode(e.target.value)}
                                                            className="h-8 text-xs uppercase"
                                                        />
                                                        <Button size="sm" className="h-8" onClick={handleVerifyGiftCard} disabled={!giftCardCode || isVerifyingGiftCard}>
                                                ...
                                                {availableGiftCards.length > 0 ? (
                                                    <div className="max-h-40 overflow-y-auto p-1">
                                                        <div className="text-[10px] uppercase font-bold text-muted-foreground px-2 py-1">Disponibles pour ce client</div>
                                                        {availableGiftCards.map(card => (
                                                            <button 
                                                                key={card.id}
                                                                onClick={() => {
                                                                    setGiftCardCode(card.code);
                                                                    // Auto verify after a short delay or allow user to click
                                                                    setTimeout(() => {
                                                                ...
                                                                className="w-full text-left px-2 py-2 hover:bg-slate-100 rounded flex justify-between items-center group/card"
                                                            >
                                                                <div>
                                                                    <div className="font-semibold text-xs text-slate-900">{card.code}</div>
                                                                    <div className="text-[10px] text-muted-foreground">Solde: {card.current_balance} €</div>
                                                                </div>
                                                                <PlusCircle className="h-3.5 w-3.5 text-primary opacity-0 group-hover/card:opacity-100 transition-opacity" />
                                                            </button>
                                                        ))}
                                                    </div>
                                                ) : (
                                                    <div className="p-3 text-center text-xs text-muted-foreground italic bg-slate-50">
                                                        Aucune carte associée à ce client.
                                                    </div>
                                                )}
                                            </PopoverContent>
                                         </Popover>
